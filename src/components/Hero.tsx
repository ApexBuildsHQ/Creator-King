...
interface HeroProps {
  ...
  activeCategory: 'all' | 'media' | 'text' | 'utility';
  onCategoryChange: (category: 'all' | 'media' | 'text' | 'utility') => void;
}
...
export function Hero({ currentLang, t, searchQuery, onSearchChange, activeCategory, onCategoryChange }: HeroProps) {
  return (
    <div className="relative py-12 md:py-20 overflow-hidden text-center max-w-5xl mx-auto px-4 sm:px-6">
      ...
      {/* Beautiful horizontal scrollable categories bar right under the input */}
      <div className="mt-6 flex justify-center w-full max-w-xs sm:max-w-md md:max-w-xl lg:max-w-2xl xl:max-w-3xl mx-auto px-4">
        <div className="w-full overflow-x-auto flex items-center justify-start sm:justify-center gap-2 py-2 px-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          {[
            ...
            <button
               ...
               className={`py-2 px-4 sm:py-2.5 sm:px-5 rounded-xl sm:rounded-2xl text-xs sm:text-sm font-bold transition select-none flex-shrink-0 cursor-pointer active:scale-95 ${
