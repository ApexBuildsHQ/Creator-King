...
export default function ScreenCameraRecorder({ t, isRtl }: ToolComponentProps) {
  ...
  const startRecording = async () => {
    ...
    if (sourceMode !== 'camera' && (!navigator.mediaDevices || typeof navigator.mediaDevices.getDisplayMedia === 'undefined')) {
      const isArabic = isRtl;
      setErrorMessage(
        isArabic
          ? 'تسجيل الشاشة غير مدعوم في متصفحك أو تحت بيئة الإطار الحالي (iframe). يرجى اختيار تسجيل الكاميرا فقط، أو فتح التطبيق في تبويب جديد.'
          : 'Screen recording is not supported in this browser or iframe. Please try choosing "Camera" mode only, or open the app in a new tab.'
      );
      return;
    }
    ...
    try {
      ...
      console.error(error);
      const isArabic = isRtl;
      let errMsg = t('recorder.error_permission');
      
      if (error && (error.name === 'NotAllowedError' || error.message?.includes('Permission denied') || error.message?.includes('NotAllowedError') || error.message?.includes('denied'))) {
        errMsg = isArabic
          ? 'تم رفض إذن الوصول إلى الشاشة أو الكاميرا/الميكروفون. يرجى تجربة تسجيل الكاميرا فقط، أو منح صلاحية الوصول من شريط عنوان المتصفح.'
          : 'Permission denied to access the screen, camera, or microphone. Try selecting "Camera Only" mode, or grant browser permissions and try again.';
      } else if (error && (error.name === 'TypeError' || error.message?.includes('not a function'))) {
        errMsg = isArabic
          ? 'تسجيل الشاشة غير مدعوم في متصفحك الحالي أو داخل هذا الإطار. يرجى تجربة اختيار تسجيل الكاميرا فقط.'
          : 'Screen recording (getDisplayMedia) is not a function or unsupported in this iframe. Please select "Camera Only" mode.';
      } else if (error && error.message) {
        errMsg = `${isArabic ? 'حدث خطأ: ' : 'Error: '} ${error.message}`;
      }
